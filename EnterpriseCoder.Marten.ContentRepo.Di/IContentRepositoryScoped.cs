...
/// uploading and downloading resources, and managing files and metadata.  This interface matches the <c>IContentRepository</c>
/// interface, but is designed to be used with a dependency injection framework where an IDocumentSession is provided by the
/// dependency injection framework.  This allows the caller to manage the session lifecycle.
/// </summary>
/// <remarks>
/// Transaction Control:  It is the responsibility of the caller to ensure that the session is properly committed or rolled back with these exceptions:
/// <list type="bullet">
...
/// commits, one page of 100 entries at a time.  This ensures that even in the case of a huge bucket, memory allocation is kept reasonable.
/// The deletion of the actual bucket is still a part of the injected IDocumentSession.</description></item>
/// </list>
/// </remarks>
public interface IContentRepositoryScoped
{
    ...
    /// </summary>
    /// <remarks>
    /// Transaction Control:  This method will create the bucket immediately using a separate database transaction.
    /// The injected session is not saved by this method.  The caller is responsible for saving the session.
    /// </remarks>
    ...
    /// on the injected IDocumentSession reference. 
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Transaction Control:  This method will insert the content into the session injected by the dependency injection framework,
    ...
    Task UploadStreamAsync(string bucketName, ContentRepositoryResourcePath resourcePath,
        ...
        Guid? userGuid = null, long userValue = 0L, IUploadProgressCallback? uploadProgressCallback = null);
    ...
    /// specified by the <paramref name="bucketName"/> and <paramref name="resourcePath"/> arguments.  If the given
    ...
    Task CreateDirectoryAsync(string bucketName, ContentRepositoryDirectory directoryPath);
    ...
    /// <exception cref="DirectoryAlreadyExistsException">Thrown when a directory already exists at the specified path.</exception>
    /// <returns>A task that represents the asynchronous operation.</returns>
    Task CreateDirectoryAsync(string bucketName, ContentRepositoryDirectory directoryPath, bool autoCreateBucket);
    ...
    /// <returns>A task that represents the asynchronous operation.</returns>
    Task DeleteDirectoryAsync(string bucketName, ContentRepositoryDirectory directoryPath);
    ...
    /// <exception cref="DirectoryNotEmptyException">Thrown when the directory is not empty and force is false.</exception>
    /// <returns>A task that represents the asynchronous operation.</returns>
    Task DeleteDirectoryAsync(string bucketName, ContentRepositoryDirectory directoryPath, bool force);
    ...
    Task<bool> DirectoryExistsAsync(string bucketName, ContentRepositoryDirectory directoryPath);
    ...
    /// <exception cref="BucketNotFoundException">Thrown when the bucket is not found.</exception>
    Task<IReadOnlyList<ContentRepositoryDirectoryInfo>> GetDirectoryListingAsync(string bucketName, ContentRepositoryDirectory directoryPath);
}