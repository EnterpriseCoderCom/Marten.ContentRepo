...
public class ContentRepositoryScoped : IContentRepositoryScoped
{
        ...
        long userValue = 0L, IUploadProgressCallback? uploadProgressCallback = null)
    {
        await _contentRepository.UploadStreamAsync(_documentSession, bucketName, resourcePath, inStream,
            autoCreateBucket,
            overwriteExisting, userGuid, userValue, uploadProgressCallback);
    }
    ...
    {
        await _contentRepository.RenameResourceAsync(_documentSession, sourceBucketName, sourceResourcePath, destinationBucketName,
    ...
    public async Task CopyResourceAsync(string sourceBucketName, ContentRepositoryResourcePath sourceResourcePath,
        string destinationBucketName,
        ContentRepositoryResourcePath destinationResourcePath,
        bool autoCreateBucket = true, bool overwriteDestination = false)
    {
        await _contentRepository.CopyResourceAsync(_documentSession, sourceBucketName, sourceResourcePath, destinationBucketName,
            destinationResourcePath,
    ...
    public async Task<PagedContentRepositoryResourceInfo> GetResourceListingAsync(string bucketName,
        ContentRepositoryDirectory resourcePrefix,
        int oneBasedPage, int pageSize, bool recursive = false)
    {
        return await _contentRepository.GetResourceListingAsync(_documentSession, bucketName, resourcePrefix, oneBasedPage,
            pageSize,
            recursive);
    }
