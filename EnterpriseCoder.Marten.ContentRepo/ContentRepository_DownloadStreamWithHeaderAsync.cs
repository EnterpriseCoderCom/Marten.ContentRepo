...
{
    public async Task<Tuple<Stream, ContentRepositoryResourceInfo>> DownloadStreamWithHeaderAsync(
        IDocumentSession documentSession, string bucketName,
        ContentRepositoryResourcePath resourcePath)
    {
        ...
        try
        {
            ...
            return new Tuple<Stream, ContentRepositoryResourceInfo>(unzipStream,
                targetHeader.ToContentFileInfoDto(bucketName));
        }
        catch (Exception)
        {
